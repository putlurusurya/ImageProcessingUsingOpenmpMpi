...
struct img IVP::create_img(std::string path){
	...
	t2=omp_get_wtime();
	printf("Time for execution for creating 2D image in parallel: %lf\n",t2-t1);
    return img1;
}
...
void IVP::print(struct img img1){
	...
	t2=omp_get_wtime();
	// printf("Time for execution for printing 2D image in parallel: %lf\n",t2-t1);
...
struct img IVP::create_filter(int n){
	...
	#pragma omp parallel num_threads(no_threads)
	{
		#pragma omp for  collapse(2) private(x,y) reduction(+:sum)
		for (x = -n/2; x <= n/2; x++) {
			for (y = -n/2; y <= n/2; y++) {
				r = sqrt(x * x + y * y);
    ...
    f.w=n;
    t2=omp_get_wtime();
    printf("Time for execution for creating filter in parallel: %lf\n",t2-t1);
    return f;
}
...
struct img IVP::blur(struct img img1,struct img f){
	...
	t2=omp_get_wtime();
	printf("Time for execution for blurring of image in parallel: %lf\n",t2-t1);
    return out;
}
...
struct img IVP::rotate(struct img img1){
	...
	t2=omp_get_wtime();
	printf("Time for execution for rotation of image in parallel: %lf\n",t2-t1);
    return out;
}
...
struct img3d IVP::create_rgb_img(string path){
	...
	#pragma omp parallel num_threads(no_threads)
	{
		#pragma omp for  collapse(3) private(i,j)
		for(i=0;i<img1.w;i++){
			for(j=0;j<img1.h;j++){
				for(k=0;k<img1.c;k++){
	...
	t2=omp_get_wtime();
	printf("Time for execution for creating 3d image in parallel: %lf\n",t2-t1);
    return img1;
}
...
void IVP::print3d(struct img3d img1){
	...
	t2=omp_get_wtime();
	printf("Time for execution for printing 3d image in parallel: %lf\n",t2-t1);
    Mat finalimg;
...
struct img3d IVP::rgb2yuv(struct img3d img1){
	...
	t2=omp_get_wtime();
    return yuv;
    printf("Time for execution for RGB to YUV image change in parallel: %lf\n",t2-t1);
}
...
struct img3d IVP::yuv2rgb(struct img3d img1){
	...
	t2=omp_get_wtime();
	printf("Time for execution for YUV to RGB image change in parallel: %lf\n",t2-t1);
    return rgb;
}
...
struct img IVP::rgb2gray(struct img3d img1){
	...
	t2=omp_get_wtime();
	printf("Time for execution for RGB to grayscale image change in parallel: %lf\n",t2-t1);
    return gray;
}
// phanis
int truncate(int x){
